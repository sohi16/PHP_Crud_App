...
if ($_GET["action"] === "insertData") {
  if (!empty($_POST["first_name"]) && !empty($_POST["last_name"]) && !empty($_POST["email"]) && !empty($_POST["country"]) && !empty($_POST["gender"]) && $_FILES["image"]["size"] != 0) {
    ...
    $sql = "INSERT INTO `users`(`User_Id`, `First_Name`, `Last_Name`, `Email`, `Image`, `Country`, `Gender`) VALUES (NULL,'$first_name','$last_name','$email','$new_name','$country','$gender')";
...
if ($_GET["action"] === "updateData") {
  if (!empty($_POST["first_name"]) && !empty($_POST["last_name"]) && !empty($_POST["email"]) && !empty($_POST["country"]) && !empty($_POST["gender"])) {
    ...
    $sql = "UPDATE `users` SET `First_Name`='$first_name',`Last_Name`='$last_name',`Email`='$email',`Image`='$new_name',`Country`='$country',`Gender`='$gender' WHERE `User_Id`=$id";
    if (mysqli_query($conn, $sql)) {
      echo json_encode([
        "statusCode" => 200,
